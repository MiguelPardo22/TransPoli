	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http
				...
				.authorizeHttpRequests(
						authorize -> authorize.requestMatchers("/register**", "/bookstores/**","/Layout/**").permitAll() // Rutas
																											// públicas
				...
				.formLogin(form -> form.loginPage("/login").defaultSuccessUrl("/index", true) // Redirigir a /index
																								// después del login
																								// exitoso
						.permitAll())
				...
				).exceptionHandling(exception -> exception.accessDeniedPage("/403") // Configuración de página de error
																					// 403
				);
		;
